...
(function () {
    ...
    C2XFunctions.prototype.config = function (config) {       
        ...
        _txtDescription = config.TxtDescription ;
        _txtProcessStart  = config.TxtProcessStart ;
        _txtProcessEnd = config.TxtProcessEnd;
    ...
    C2XFunctions.prototype.createValidator = function(){
        ...
        $("#OperationAddressForm").kendoValidator({
            messages: {              
                ...
                required: function(input){c2xProcAddress.getRequiredMessage(input);}               
            }
        });
    };
    ...
    function getData() {
        ...
        if ((typeof (_currentEditing) != "undefined") && (_currentEditing != null)) {
            ...
            processID = _currentEditing.ProcessID;
        ...
        var txtDescription = $("#" + _txtDescription).val();
        ...
        var start = $find('ProcessStart');
        var end = $find('ProcessEnd');
        //start.set_selectedDate(new Date(parseInt(model.ProcessStart.replace('/Date(', ''))));
        //end.set_selectedDate(new Date(parseInt(model.ProcessEnd.replace('/Date(', ''))));
        var model = {
            uid:gUid,
            UID: uid,
            ...
            ProcessStart: start.get_selectedDate(),
            ProcessEnd: end.get_selectedDate(),
    ...
    function bindDataItem(model) {
        ...
        if (model != null) {
            ...
            $("#" +_txtDescription).val(model.Description);
            //$("#" +_txtProcessStart).val(model.TxtProcessStart);
            //$("#" +_txtProcessEnd).val(model.TxtProcessEnd);
            var start = $find('ProcessStart');
            var end = $find('ProcessEnd');
            //console.log(model.ProcessEnd)
            if (model.ProcessStart != null)
            {
                //start.set_selectedDate(new Date(parseInt(model.ProcessStart.replace('/Date(', ''))));
                start.set_selectedDate( model.ProcessStart );
            }
            if (model.ProcessEnd != null) {
                //end.set_selectedDate(new Date(parseInt(model.ProcessEnd.replace('/Date(', ''))));
                end.set_selectedDate( model.ProcessEnd );
            }
           
            //console.log(new Date(parseInt(model.ProcessStart.replace('/Date(', ''))));
    ...
    c2xProcAddress = new C2XFunctions();
})();