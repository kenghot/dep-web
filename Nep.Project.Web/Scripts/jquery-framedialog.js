...
(function ($) {
    ...
    //helper function to get response value	from the parent
    //	uid - child id, or empty for current FrameDialog
    $.FrameDialog.getResult = function (uid) {
        if (uid) {
            //get child value
            return jQuery.FrameDialog._results[uid];
        } else {
            //get value from inside
            var uid = $.FrameDialog._getUid();

            if (uid != null && window.parent && window.parent.jQuery && window.parent.jQuery.FrameDialog && window.parent.jQuery.FrameDialog._results) {
                return window.parent.jQuery.FrameDialog._results[uid];
            }
        }
    }
    ...
    $.fn.getResult = function () {
        return $.FrameDialog.getResult($(this).attr("id"));
    }
